...
3) An NP definition. At present, you can only use simple NPs and the auto-np button does nothing (but leave it checked). You can set the radius of the NP, the value of the electrostatic potential at the surface in mV and choose a material from all the ones it found in the MaterialSet.csv . You can load in extra materials with the Load Material Set button if you know what you're doing and have another prepared - "surface-pmfp/MaterialSetPMFP.csv" would be a typical choice here to load in the materials generated via machine learning.
